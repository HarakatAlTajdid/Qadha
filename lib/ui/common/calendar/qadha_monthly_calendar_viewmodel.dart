...
class QadhaMonthlyCalendarViewModel extends BaseViewModel {
  ...
  void setFrame(int increment) {
    currentFrame = DateTime(currentFrame.year,
        currentFrame.month + increment);
    getDaysForCurrentFrame();
  }
  ...
  void getDaysForCurrentFrame() {
    ...
    final currentFrameDaysCount = DateTime(currentFrame.year, currentFrame.month + 1, 0).day;
    final currentFrameFirstDay = DateTime.utc(currentFrame.year, currentFrame.month, 1).weekday;
    ...
    if (currentFrameFirstDay != 1) {
      for (int i = 1; i < currentFrameFirstDay; i++) {
        final day = currentFrame.subtract(Duration(days: currentFrameFirstDay - i));
        days.add(day);
      }
    }
    ...
    if (days.length < 42) {
      ...
      for (int i = 0; i < missingDaysCount; i++) {
        final day = DateTime(days.last.year, days.last.month, days.last.day + 1);
        days.add(day);
      }
    }
